...
Метод get используется для получения значения по ключу, но предупреждает если ключ выведен неверно.
...
Чтобы метод get() возвращал дефолтное значение, отличное от None, нужно передать его вторым аргументом. Например:
...
Чтобы из всего этого седлать список понадобиться функция list()
...
Если нужно перебрать и ключи и значение можно использовать метод items()
...
так код будет более читабельным и коротким.

Также, если не указывать по чем будет перебирать, по умолчанию будуь ключи:
...
Чтобы искать какое то значение в словаре, можно использовать условный оператор if/elif/else.
    shopping_list = {
        'молоко': 'milk',
        'сахар': 'sugar',
        'мука': 'flour',
...
А чтобы потвердить что какого то значения нету можно применить логический оператор not.
